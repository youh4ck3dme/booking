...
export const MyBookings: React.FC = () => {
    ...
    const BookingItem = ({ booking, isUpcoming = false }: { booking: Booking, isUpcoming?: boolean }) => (
        <Card className="mb-md">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-md">
                <div className="space-y-xs">
                    <div className="flex items-center gap-sm text-primary font-bold">
                        <Calendar size={18} />
                        <span>{format(new Date(booking.date), 'd. MMMM yyyy', { locale: sk })}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-sm text-secondary">
                        <UserIcon size={18} />
                        <span>{booking.serviceName} s {booking.employeeName}</span>
                    </div>
                </div>

                <div className="flex flex-col items-end gap-sm">
                    <div className={`px-sm py-xs rounded-full text-xs font-bold uppercase tracking-wider ${booking.status === 'confirmed' ? 'bg-green-500/20 text-green-400' :
                        booking.status === 'pending' ? 'bg-yellow-500/20 text-yellow-400' :
                            'bg-red-500/20 text-red-400'
                        }`}>
                        {booking.status === 'confirmed' ? 'Potvrdené' :
                            booking.status === 'pending' ? 'Čaká na potvrdenie' : 'Zrušené'}
                    </div>
        ...
        <div className="container py-xl max-w-4xl mx-auto">
            ...
            >
                ...
                {isLoading ? (
                    ...
                    <div className="text-center py-xl glass-card">
                        <p className="text-secondary mb-md">Zatiaľ nemáte žiadne rezervácie.</p>
                        <Button onClick={() => window.location.href = '/book'}>Vytvoriť rezerváciu</Button>
                    </div>
                ) : (
                    <div className="space-y-xl">
                        <section>
                            ...
                            {upcomingBookings.length > 0 ? (
                                <div className="space-y-md">
                                    {upcomingBookings.map(b => <BookingItem key={b.id} booking={b} isUpcoming />)}
                                </div>
                            ) : (
                                <p className="text-secondary italic">Žiadne nadchádzajúce rezervácie.</p>
                            )}
                        </section>
                        ...
                        {cancelledBookings.length > 0 && (
                            <section className="opacity-50">
                                ...
                                <div className="space-y-md">
                                    {cancelledBookings.map(b => <BookingItem key={b.id} booking={b} />)}
                                </div>
                            </section>
                        )}
                    </div>
