...
test.describe('Booking Flow', () => {
    ...
    test('should create a new booking', async ({ page }) => {
        ...
        // 1. Select Service
        console.log('Waiting for service selection...');
        // Service name is now in Card components
        ...
        // Wait for employee selection step
        await expect(page.locator('text=Vyberte pracovníka')).toBeVisible({ timeout: 10000 });

        // 2. Select Employee - use the "Ktokoľvek" (Anyone) button
        const anyoneButton = page.locator('button').filter({ hasText: 'Ktokoľvek' });
        await expect(anyoneButton).toBeVisible();
        await anyoneButton.click();
